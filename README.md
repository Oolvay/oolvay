...
6. [Third-Party Service Setup](#6-third-party-service-setup)
   ...
   - [Arcjet (Rate Limiting & Bot Protection)](#66-arcjet-rate-limiting--bot-protection)
...
13. [Blog — MDX System](#13-blog--mdx-system)
...
- Passkeys (WebAuthn — biometric / device authentication)
...
- React Email preview server for local development

### Storage

- AWS S3 avatar uploads
- Pre-signed URLs — browser uploads directly to S3, never through your server
- IAM roles provisioned via AWS CDK — no long-lived access keys
...
- Admin-only blog write / edit interface
...
### Pages (Scaffolded & Ready to Customise)
...
| Session Caching                | Upstash Redis                | Latest  |
...
| Rate Limiting / Bot Protection | Arcjet                       | ^1.3.1  |
| Error Monitoring               | Sentry                       | 10.x    |
| Analytics                      | PostHog                      | Latest  |
| Payments (global)              | LemonSqueezy                 | —       |
...
│       ├── price-map.ts         # Internal ID → provider ID mapping
...
- [Better Auth](https://better-auth.com) — authentication (self-hosted, no account needed)
...
To run Drizzle Studio (visual database browser):
...
3. Choose **"Regional"**, select the same region as your Neon database.
...
> **Note on Passkeys:** Passkeys (WebAuthn) work out of the box without any configuration — they use your app's own domain as the Relying Party identifier. No third-party keys required.
...
Oolvay uses AWS CDK to provision all infrastructure as code. This means you do not manually click through the AWS console — you run a single deploy command and CDK creates everything: the S3 bucket, the IAM role for uploads, the CloudFront distribution, and the bucket policy.
...
CDK bootstrapping provisions a small set of resources in your AWS account that CDK needs to operate. This is a one-time operation.
...
### 6.6 Arcjet (Rate Limiting & Bot Protection)
...
> **If `ARCJET_KEY` is absent**, all Arcjet guards silently no-op. The app remains functional — just unprotected. This is fine for local development but must be set before going to production.
...
2. Click **"Create Project"**, choose **"Next.js"**.
...
2. Create a new project. Choose **"Cloud (EU)"** if your users are primarily in Europe, **"Cloud (US)"** otherwise.
...
2. Create **"Pro — Monthly"** as a subscription, price $X/month.
...
> **Test mode:** LemonSqueezy has a test mode toggle in the top bar. Create all products and webhooks in test mode first. Switch to live mode only before production launch.
...
> **Note on the Razorpay checkout modal:** Unlike LemonSqueezy which redirects to a hosted page, Razorpay opens a JavaScript modal on your own domain. The `<CheckoutButton>` component handles both flows automatically — no configuration needed.
...
- Sentry → (no URL change needed)

#### Cron jobs (Vercel)

The webhook reconciliation cron is configured in `vercel.json`. It runs once per hour and re-processes any failed webhook events from the past 24 hours. It is protected by a `CRON_SECRET` environment variable — add this to Vercel as well:
...
Copy this block into `.env.local` and fill in each value by following Section 6.
...
**LemonSqueezy and Razorpay** — use [ngrok](https://ngrok.com) to expose your local server:
...
Email templates live in `emails/`. They are written in React using the React Email component library, which gives you full JSX control over HTML email layout.
...
3. Preview it with `bun email:dev`.
4. Send it from a server action or API route:
...
The following statuses are considered "active" for the purpose of access control:

- `active`
- `trialing`
- `past_due` (grace period — do not lock out users immediately on failed payment)
...
| `draft` | No | `true` hides the post from public listing; visible in admin drafts view |
...
### Admin write / edit interface

The blog write and edit UI is available at `/admin/blog/new` and `/admin/blog/[slug]/edit`. It is protected by the `admin` role — only users with `role = "admin"` in the database can access it.
...
2. **Type check** — runs `tsc --noEmit` in strict mode
3. **Build** — runs `bun run build`; fails the pipeline if the build fails
4. **Deploy** — triggers a Vercel deployment on successful builds to `main`
...
- [ ] **Vercel environment variables** — Mirror production keys in Vercel dashboard
- [ ] **Google OAuth** — Add production redirect URI
- [ ] **LemonSqueezy or Razorpay** — Switch to live mode / live keys
...
- If a migration was partially applied, check the `drizzle/__drizzle_migrations` table for the last successful migration.
...
- The raw body must be read with `Buffer.from(await req.arrayBuffer())` — reading it as JSON first will break signature verification.
